...
- Thay toàn bộ icon ứng dụng bằng bộ mã vàng lớn trên nền bàn cờ xanh, giữ nhận diện rõ ở kích thước nhỏ.
- Panel Phân tích nước đi mặc định hiển thị phương án Best; phương án #2 chỉ mở thêm khi người dùng cần.
- Chế độ xem biến Stockfish co bàn cờ theo chiều cao khả dụng để cụm điều khiển không tràn khỏi panel chính.
...
| 0.10.0 | Insights, Batch & Sync | Insights Dashboard + report card/estimated Elo; phân tích toàn ván hàng loạt; đồng bộ tăng dần theo watermark | 18–24 ngày |
...
Tiến độ theo số milestone phát hành: **7/10 hoàn thành (70%)**. Con số này chỉ thể hiện số mốc, không quy đổi theo độ lớn công việc.
...
## 10. Milestone 0.10.0 — Insights, Batch Analysis & Sync

### Câu chuyện người dùng

> Sau khi phân tích ván, tôi muốn một "phiếu điểm" ngắn cho biết mình chơi giai đoạn nào tốt/kém và tương đương trình độ nào; đồng thời nhìn được xu hướng qua nhiều ván — opening nào đang ăn điểm, khung giờ nào tôi chơi kém. Tôi cũng muốn phân tích cả loạt ván một lần thay vì bấm từng ván, và mỗi lần đồng bộ lấy đủ ván mới mà không trùng hay bỏ sót.

### Mục tiêu

Biến dữ liệu đã có (cache phân tích theo ply, PGN, `%clk`, hồ sơ) thành thông tin dễ hành động, hoàn toàn offline và không gọi LLM. Milestone gồm ba nhóm bổ trợ nhau: kéo đủ dữ liệu (Nhóm C — sync), phân tích hàng loạt (Nhóm B — batch), rồi trực quan hóa (Nhóm A — insights). Học ý tưởng từ chess.com Insights/Game Review và OpenChess-Insights nhưng chỉ dùng số liệu truy ngược được về FEN/ply/ván.

### Thứ tự làm & phụ thuộc

Chuỗi giá trị: **C (sync) → B (batch) → A (insights)** — sync-until-watermark kéo đủ ván; batch phân tích cả loạt; insights trực quan hóa dữ liệu đã phân tích. Report card (thuộc Nhóm A) là phần nhẹ nhất, không phụ thuộc B/C nên có thể làm sớm song song.

### Nhóm A — Performance Insights & Report Card

#### A1 — Report card mỗi ván + estimated Elo (nhẹ, làm sớm)

- Gắn vào **modal "Tổng kết ván đấu"** (`src/app/components/GameInsightsModals.tsx`, nhánh `summaryOpen`). Chèn card "Phiếu điểm" mới ngay dưới `summary-heading`, phía trên `summary-players`.
- Nội dung card:
  - **Estimated Elo màn trình diễn** hiển thị dạng số lớn kèm nhãn "≈ ước lượng" và tooltip nói rõ tính từ ACPL/chất lượng nước, không phải Elo thật.
  - Ba chip giai đoạn Khai cuộc / Trung cuộc / Tàn cuộc, mỗi chip có hạng + ACPL, màu xanh→đỏ theo mức lỗi.
  - Một dòng tóm tắt.
- Chỉ hiển thị cho **bên của hồ sơ đang chọn** (khớp username với White/Black, tái dùng `resolveStoryPerspective` và logic `PlayerMoveStats`); ẩn khi không xác định được màu.
- Estimated Elo và hạng giai đoạn tính **deterministic** từ ACPL/chất lượng nước từng giai đoạn (đã có trong `fullGameSummary`/phase breakdown); cùng một ván luôn cho cùng kết quả.
- Click chip giai đoạn nhảy tới nước đầu giai đoạn trên bàn (`onOpenIndex`, tùy chọn). Dòng tóm tắt deterministic; nếu có API key thì HLV AI chỉ diễn đạt số đã khóa (tái dùng `game-coach-card`, giữ validator + fallback).

#### A2 — Insights Dashboard (analytics hành vi)

Mở rộng **modal "Tiến bộ"** (`GameInsightsModals.tsx`, nhánh `dashboardOpen`) bằng **thanh tab** ở đầu modal, ba tab để mỗi màn vẫn scan nhanh:

- **Chất lượng** — giữ nguyên nội dung dashboard hiện tại: metrics, biểu đồ ACPL 20 ván, breakdown theo giai đoạn/màu/thể loại/khai cuộc, chủ đề cần ưu tiên.
- **Kết quả** — bảng opening: số ván · Thắng/Hòa/Bại · win% · ACPL, lọc theo time class; personal move explorer (nước đầu hay chơi nhất + win% theo màu).
- **Nhịp độ** — win% theo giờ trong ngày / thứ trong tuần / độ dài ván; gộp phần "Quản lý thời gian" hiện có vào đây.

#### Tầng dữ liệu cần bổ sung

- `DashboardMoveRecord` (`src/dashboard.ts`) hiện ở mức nước đi, chưa có kết quả ván (W/D/L), giờ bắt đầu ván và nước đầu. Bổ sung ở tầng Rust query/repository (`src/features/library`, `src-tauri`): game `Result`, `UTCDate`+`UTCTime` (fallback `Date`+`StartTime`), nước đầu mỗi ván.
- Nếu PGN thiếu giờ thì ẩn phần time-of-day (degrade như cách xử lý `%clk`). Tính on-the-fly từ dữ liệu đã lưu; chỉ thêm cột/bảng SQLite khi bắt buộc và phải tạo lại được.

### Nhóm B — Batch Analysis (phân tích toàn ván hàng loạt)

- **Vấn đề:** hiện phải bấm "Phân tích toàn ván" thủ công từng ván (`FullGameAnalysisAction`), rất mất công khi có nhiều ván mới.
- **Điểm vào:** nút "Phân tích hàng loạt" ở Kho ván (sidebar + modal, cạnh nút Làm mới); hành động phụ trên `syncNotice` sau khi sync ("Phân tích N ván mới"); và đổi empty-state của modal "Tiến bộ"/Insights sang "Phân tích N ván chưa phân tích".
...
- **Vấn đề:** `syncRecentGames` hiện luôn lấy đúng 20 ván mới nhất → **bỏ sót** nếu chơi >20 ván giữa 2 lần; và "sync-until-known" ngây thơ (dừng khi gặp id đã biết) sẽ **để lại lỗ hổng vĩnh viễn** khi bị ngắt giữa chừng.
- **Watermark:** mỗi hồ sơ lưu mốc thời gian ván mới nhất đã đồng bộ **liền mạch** (bất biến: mọi ván ≤ watermark đều đã có). Tận dụng `last_synced`/`markSynced` sẵn có.
- **UI tab "Đồng bộ gần đây":** hiện "Đã đồng bộ liền mạch tới: <thời gian>"; chọn chế độ **Ván mới kể từ lần trước** (mặc định, incremental) / **Số lượng cụ thể** (lần đầu / bắt kịp); ô cận trên **Tối đa mỗi lần** để không chạy vô hạn.
- **Phân trang tuần tự:** chess.com đi lùi từng tháng và lọc `end_time > watermark`; lichess dùng `since=watermark`. Dừng khi chạm watermark **hoặc** chạm cận trên.
- **Ba kết quả (`syncNotice`):**
  1. **Liền mạch** → watermark **tiến** tới ván mới nhất.
  2. **Bị ngắt** (429 kéo dài / lỗi mạng) trước khi chạm watermark → **vẫn lưu** phần đã tải (dedup lo phần trùng), watermark **GIỮ NGUYÊN**, hiện badge "còn khoảng trống" + nút "Đồng bộ tiếp".
  3. **Chạm cận trên** → xử lý như (2).
- **Tôn trọng nền tảng:** lichess 429 → đếm ngược 60s tự tiếp (giữ tuần tự); chess.com giữ tuần tự (serial access không bị rate-limit) + user-agent có liên hệ (code đã có).
- **Dedup:** vẫn dựa trên `id = SHA256(PGN chuẩn hóa)` với `ON CONFLICT(id) DO UPDATE` (đã có) — tải trùng chỉ upsert metadata, không tạo bản trùng, không mất phân tích cũ.
- **Chỉ báo trực quan:** badge "còn khoảng trống" bám vào bộ chọn hồ sơ và tab Đồng bộ cho tới khi một lần sync liền mạch lấp xong.

### Nguyên tắc

- 100% offline, không bắt buộc API key; AI chỉ diễn đạt số đã khóa; mọi con số truy ngược được về FEN/ply/ván (đúng §2).
- Không chạy lại Stockfish để dựng Insights nếu cache đã đủ.
- Batch và sync đều tuần tự: tôn trọng ngân sách "1 tác vụ Stockfish nặng/lần" (§15) và giới hạn tốc độ của nền tảng nguồn.

### Tiêu chí nghiệm thu

Insights & Report card:

- Report card và Insights hoạt động khi chưa cấu hình AI.
- Cùng một ván luôn cho cùng report card + estimated Elo (deterministic).
- W/D/L theo opening và win% theo giờ khớp lịch sử của đúng hồ sơ đang chọn.
- Mở Insights không chạy lại Stockfish nếu cache đủ; giữ ngân sách dưới 500 ms với 1.000 ván local.
- Ván PGN thiếu giờ vẫn xem được các tab khác, chỉ ẩn phần time-of-day.

Batch analysis:

- Batch chạy tuần tự, không quá 1 tác vụ Stockfish nặng cùng lúc; ván đã phân tích bị bỏ qua tức thì.
- Tạm dừng/Dừng giữ nguyên phần đã xong; đóng app rồi chạy lại không phân tích lại ván đã xong.
- Batch không tự gọi AI; UI không bị chặn khi batch đang chạy.

Sync-until-watermark:

- Chơi >20 ván giữa hai lần vẫn lấy đủ khi sync liền mạch (không bỏ sót).
- Bị ngắt giữa chừng: không mất ván đã tải, không tạo lỗ hổng vĩnh viễn; watermark chỉ tiến khi dải ván liền mạch hoàn tất.
- Lichess 429 được tôn trọng (chờ rồi tự tiếp); tải lại ván đã có không tạo bản trùng và không mất phân tích cũ.
...
## 16. Telemetry và quyền riêng tư
...
8. [ ] Insights + report card, batch analysis, sync-until-watermark — v0.10.0.
